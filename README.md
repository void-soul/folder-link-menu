...
- 迁移完成后，在原位置创建指向新位置的链接：默认优先 **Junction**（`mklink /J`，免管理员）；跨盘时 Junction 不支持，自动回退 **Symlink**（`mklink /D`，支持跨卷）
- 确认框提供「强制使用 Symlink」选项：当目标被 dsh 等要求 symlink 的跨平台工具使用时勾选，避免其因识别不到 symlink 而报错
